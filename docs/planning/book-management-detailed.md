...
### Обновление счетчиков категорий (2025-01-27)

**Проблема:** 
- Счетчики категорий не обновлялись после создания новой книги
- Несоответствие между количеством книг в категории и счетчиком
- Некоторые категории не отображались в левом меню, хотя книги в них были

**Решение:**
1. ✅ Добавлен вызов `loadData()` в `handleBookCreateComplete` для обновления категорий после создания книги
2. ✅ Исправлена логика подсчета `books_count` для подкатегорий в `CategoryTreeSerializer.get_subcategories()`:
   - Теперь всегда аннотируем `books_count`, даже если библиотеки не указаны
   - Улучшена логика использования prefetch-аннотаций из метода `tree()`
   - Правильная обработка случаев, когда prefetch уже содержит аннотацию
3. ✅ Исправлена логика отображения категорий:
   - Подкатегории корректно аннотируются и отображаются в меню
   - Родительские категории показываются, если есть книги в них или в подкатегориях
   - Категории с книгами появляются в меню, даже если книги только в подкатегориях

**Файлы изменены:**
- `frontend/src/pages/MainPage.js` - добавлен вызов `loadData()` в `handleBookCreateComplete`
- `backend/books/serializers.py` - исправлена логика в `CategoryTreeSerializer.get_subcategories()`

**Тестирование:**
- [x] Счетчики категорий обновляются после создания книги
- [x] Подкатегории правильно подсчитываются и отображаются
- [x] Категории с книгами появляются в меню
- [x] Счетчики соответствуют реальному количеству книг в категориях

---

### Объединение прогресс-баров нормализации и анализа (2025-01-27)

**Проблема:**
- Два отдельных прогресс-бара (нормализация и LLM анализ) показывались последовательно
- Пользователь видел два разных процесса вместо одного объединенного

**Решение:**
1. ✅ Объединены два прогресс-бара в один общий прогресс-бар
2. ✅ Нормализация занимает 0-40% общего прогресса
3. ✅ LLM анализ занимает 40-100% общего прогресса
4. ✅ Текст прогресс-бара динамически меняется: "Нормализация страниц..." → "Анализ страниц LLM..."
5. ✅ Картинки по-прежнему обновляются сразу после нормализации

**Файлы изменены:**
- `frontend/src/components/UploadPagesStep.js` - добавлены состояния `overallProgress` и `progressText`, объединена логика прогресс-баров

**Тестирование:**
- [x] Объединенный прогресс-бар отображается корректно
- [x] Прогресс плавно переходит от нормализации к анализу
- [x] Текст обновляется в зависимости от этапа
- [x] Картинки обновляются после нормализации

---

### Исправление ошибки создания книги (400 Bad Request) (2025-01-27)

**Проблема:**
- При создании книги возникала ошибка 400 Bad Request
- Ошибка валидации в поле `isbn`: значение превышало максимальную длину (20 символов)
- LLM возвращал несколько ISBN через запятую (например, "5-17-040867-6, 5-271-15458-0"), что превышало лимит модели

**Решение:**
1. ✅ Улучшена обработка ошибок на фронтенде:
   - Добавлен детальный вывод ошибок валидации в консоль и интерфейс
   - Показываются все поля с ошибками валидации в понятном формате
2. ✅ Добавлено логирование на бэкенде:
   - Логирование входящих данных запроса в `BookViewSet.create`
   - Логирование ошибок валидации перед отправкой ответа
3. ✅ Исправлена обработка ISBN:
   - Если ISBN содержит запятую, берется только первый ISBN (до запятой)
   - Значение обрезается до 20 символов (максимальная длина поля в модели)
   - Убираются пробелы с помощью `trim()`
4. ✅ Улучшена обработка данных:
   - `normalized_image_urls` всегда передается как массив (не `null`)
   - Добавлена проверка на `NaN` для числовых полей (`year`, `circulation`)
   - `cover_page_index` удаляется, если нет страниц (изображений)
   - Улучшена обработка пустых значений

**Файлы изменены:**
- `frontend/src/components/BookCreateWizard.js` - улучшена обработка ISBN и данных, добавлено логирование
- `frontend/src/components/BookFormStep.js` - улучшена обработка ошибок валидации
- `backend/books/views/books.py` - добавлено логирование в метод `create`

**Тестирование:**
- [x] Ошибки валидации отображаются детально в интерфейсе
- [x] ISBN корректно обрабатывается (берется первый, обрезается до 20 символов)
- [x] Логирование работает на фронтенде и бэкенде
- [x] Книги создаются успешно с корректными данными
...
## ВАЖНОЕ УТОЧНЕНИЕ: Страницы и изображения - это одно и то же

**Дата уточнения:** 2025-01-27

**Проблема:** В плане была путаница между "страницами книги" и "изображениями книги".

**Решение:**
- **Страницы книги (`BookPage`) = Изображения книги** - это одно и то же
- Отдельной модели `BookImage` не требуется (хотя она существует в коде, но не используется)
- Все изображения управляются через страницы (`BookPage`)
- Endpoints `/api/books/{id}/images/` существуют в коде, но не должны использоваться
- Все изображения добавляются/удаляются через endpoints для страниц (`/api/books/{id}/pages/`)

**Что уже реализовано правильно:**
- ✅ Загрузка страниц в BookCreateWizard работает корректно
- ✅ Отображение страниц в BookDetailModal работает корректно
- ✅ Управление страницами в BookEditModal работает корректно
- ✅ Обложка книги (`cover_page`) выбирается из страниц (`BookPage`)

**Что НЕ нужно реализовывать:**
- ❌ Отдельный функционал для добавления "изображений" (BookImage)
- ❌ UI для управления изображениями отдельно от страниц
- ❌ Endpoints `/api/books/{id}/images/` не должны использоваться в UI

**Итог:** План обновлен. Все упоминания об отдельном управлении "изображениями" удалены. Страницы = изображения.

---

### Исправление темной темы для блока добавления электронных версий (2025-01-27)

**Проблема:**
- Блок добавления электронной версии был реализован с inline стилями в светлой теме
- При включении темной темы блок оставался светлым и не адаптировался

**Решение:**
1. ✅ Добавлены CSS классы для всех элементов блока добавления электронных версий
2. ✅ Добавлены стили для темной темы (`.dark-theme`) для всех элементов:
   - Форма добавления (темный фон `#333`, темные границы `#555`)
   - Поля ввода (темный фон `#2a2a2a`, светлый текст)
   - Метки (светлый текст `#ddd`)
   - Кнопки (адаптированные цвета для disabled состояния)
   - Список электронных версий (темный фон, светлый текст)
3. ✅ Заменены все inline стили на CSS классы в компоненте `BookEditModal.js`

**Файлы изменены:**
- `frontend/src/components/BookEditModal.js` - заменены inline стили на CSS классы
- `frontend/src/components/BookEditModal.css` - добавлены стили для блока электронных версий и темной темы

**Тестирование:**
- [x] Блок добавления электронных версий корректно отображается в светлой теме
- [x] Блок добавления электронных версий корректно отображается в темной теме
- [x] Все элементы формы адаптируются к выбранной теме
- [x] Поля ввода имеют правильные цвета в темной теме
- [x] Кнопки имеют правильные цвета в темной теме
- [x] Список электронных версий корректно отображается в темной теме

---

### Исправление загрузки библиотек в BookTransferModal (2025-01-27)

**Проблема:**
- При попытке передать книгу вылетала ошибка "У вас нет других библиотек для передачи книги"
- Загружались только библиотеки текущего пользователя (`getMyLibraries()`)
- Книгу должно быть можно передать в любую библиотеку любого пользователя, а не только в свою другую библиотеку

**Решение:**
1. ✅ Изменен API вызов с `librariesAPI.getMyLibraries()` на `librariesAPI.getAll()` для загрузки всех библиотек всех пользователей
2. ✅ Обновлен текст ошибки: "Нет доступных библиотек для передачи книги" (вместо "У вас нет других библиотек")
3. ✅ Добавлено отображение владельца библиотеки в списке выбора: `{library.name} (${library.city}) — ${library.owner_username}`
4. ✅ Текущая библиотека книги (если она есть) исключается из списка, чтобы не передавать книгу в ту же библиотеку

**Файлы изменены:**
- `frontend/src/components/BookTransferModal.js` - изменен API вызов и добавлено отображение владельца библиотеки

**Тестирование:**
- [x] Загружаются все библиотеки всех пользователей
- [x] Текущая библиотека книги исключается из списка
- [x] Отображается владелец библиотеки в списке выбора
- [x] Книгу можно передать в библиотеку любого пользователя
- [x] Ошибка не появляется, если есть доступные библиотеки
