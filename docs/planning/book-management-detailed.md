...
15. ✅ Обновить фабрику для генерации дат прочтения для книг со статусом 'read'
...
- [x] Для прочитанных книг (status='read') отображается дата первого прочтения рядом со статусом
- [x] Фабрика генерирует даты прочтения (от 1 до 5) для книг со статусом 'read'
...
- [ ] Ошибки обработки обрабатываются корректно (через ConfirmModal, не через alert)
...
13. Реализовать добавление дат прочтения для книг со статусом 'read' (можно добавить несколько дат для отслеживания перечитываний)
...
- [ ] Добавление дат прочтения для книг со статусом 'read' работает (можно добавить несколько дат)
...
8. Реализовать управление статусом книги и датами прочтения (изменение статуса, добавление/удаление дат прочтения для статуса 'read')
...
- [ ] Добавление/удаление дат прочтения работает (для книг со статусом 'read')
...
2. **Этап 2: BookCreateWizard - Шаг 1** → Тестирование → ⏳ ОЖИДАЕТ
3. **Этап 3: BookCreateWizard - Шаг 2** → Тестирование → ⏳ ОЖИДАЕТ
