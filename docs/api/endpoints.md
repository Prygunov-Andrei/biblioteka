# 📡 API Endpoints - Полная документация

## Базовый URL
```
http://localhost:8000/api/
```

---

## 1. Categories (Категории)

### Список категорий
```
GET /api/categories/
```
**Query параметры:**
- `search` - поиск по названию или коду

**Ответ:** `200 OK`
```json
[
  {
    "id": 1,
    "code": "t2mv",
    "name": "2-я Мировая война",
    "slug": "2-ya-mirovaya-voyna",
    "icon": "📚",
    "order": 1,
    "books_count": 5
  }
]
```

### Создание категории
```
POST /api/categories/
```
**Body:**
```json
{
  "code": "test",
  "name": "Test Category",
  "slug": "test-category",
  "icon": "📚",
  "order": 0
}
```

### Детали категории
```
GET /api/categories/{slug}/
```

### Обновление категории
```
PUT /api/categories/{slug}/
PATCH /api/categories/{slug}/
```

### Удаление категории
```
DELETE /api/categories/{slug}/
```

### Дерево категорий (с подкатегориями)
```
GET /api/categories/tree/
```
**Query параметры:**
- `libraries` - список ID библиотек для фильтрации подсчета книг (можно указать несколько раз: `?libraries=1&libraries=2`)
  - Если указаны библиотеки, `books_count` будет содержать только книги из этих библиотек
  - Категории и подкатегории с нулевым количеством книг автоматически исключаются из ответа
  - **ВАЖНО:** Если библиотеки не указаны, возвращается пустой список (для левого меню)

**Использование:** Для отображения категорий в левом меню с фильтрацией по выбранным библиотекам.

### Все категории (для выбора при создании книги)
```
GET /api/categories/tree/all/
```
**Описание:**
- Возвращает ВСЕ категории в виде дерева независимо от библиотек
- Все категории возвращаются, даже если в них нет книг
- Не фильтрует по библиотекам

**Использование:** Для выбора категории при создании книги. Пользователь должен иметь возможность выбрать любую категорию из системы, даже если в ней еще нет книг.

**Ответ:** `200 OK`
```json
[
  {
    "id": 1,
    "code": "aziya",
    "name": "Азия",
    "slug": "aziya",
    "icon": "📚",
    "order": 1,
    "books_count": 42,
    "subcategories": [
      {
        "id": 2,
        "code": "tind",
        "name": "Индия, Пакистан, Тибет, Цейлон (Шри-Ланка)",
        "slug": "indiya-pakistan-tibet",
        "icon": "📚",
        "order": 6,
        "books_count": 6
      }
    ]
  }
]
```

**Примечание:** Endpoint оптимизирован для быстрого подсчета книг. Использует один SQL запрос с `annotate` вместо множественных запросов.

---

## 2. Authors (Авторы)

### Список авторов
```
GET /api/authors/
```
**Query параметры:**
- `search` - поиск по ФИО
- `birth_year_min` - минимальный год рождения
- `birth_year_max` - максимальный год рождения
- `ordering` - сортировка (по умолчанию: `full_name`)

**Ответ:** `200 OK`
```json
[
  {
    "id": 1,
    "full_name": "Иванов И.И.",
    "birth_year": 1950,
    "death_year": 2020,
    "biography": "...",
    "notes": "..."
  }
]
```

### Создание автора
```
POST /api/authors/
```
**Body:**
```json
{
  "full_name": "Петров П.П.",
  "birth_year": 1960,
  "death_year": null,
  "biography": "Биография автора",
  "notes": "Заметки"
}
```

### Детали автора
```
GET /api/authors/{id}/
```

### Книги автора
```
GET /api/authors/{id}/books/
```
**Ответ:** Список всех книг автора

### Обновление автора
```
PUT /api/authors/{id}/
PATCH /api/authors/{id}/
```

### Удаление автора
```
DELETE /api/authors/{id}/
```

---

## 3. Publishers (Издательства)

### Список издательств
```
GET /api/publishers/
```
**Query параметры:**
- `search` - поиск по названию или городу
- `city` - фильтр по городу
- `ordering` - сортировка (по умолчанию: `name`)

### Создание издательства
```
POST /api/publishers/
```
**Body:**
```json
{
  "name": "Худ. лит-ра",
  "city": "Москва",
  "website": "https://example.com",
  "description": "Описание"
}
```

### Детали издательства
```
GET /api/publishers/{id}/
```

### Книги издательства
```
GET /api/publishers/{id}/books/
```

### Обновление издательства
```
PUT /api/publishers/{id}/
PATCH /api/publishers/{id}/
```

### Удаление издательства
```
DELETE /api/publishers/{id}/
```

---

## 4. Books (Книги)

### Список книг
```
GET /api/books/
```
**Query параметры:**
- `category` - фильтр по категории (slug или ID)
- `author` - фильтр по автору (ID или имя)
- `publisher` - фильтр по издательству (ID или название)
- `libraries` - фильтр по библиотекам (список ID, например: `?libraries=1&libraries=2`)
- `year_min` - минимальный год издания
- `year_max` - максимальный год издания
- `binding_type` - тип переплета
- `format` - формат книги
- `condition` - состояние книги
- `price_min` - минимальная цена
- `price_max` - максимальная цена
- `search` - поиск по названию, подзаголовку, ISBN, автору (нечувствителен к регистру)
- `ordering` - сортировка (по умолчанию: `-created_at`)
- `page` - номер страницы (применяется автоматически если книг > 30)

**Пагинация:**
- Если книг ≤ 30: возвращается полный список, `paginated: false`
- Если книг > 30: применяется пагинация по 30 книг на страницу, `paginated: true`
- Используйте параметр `page` для перехода на следующую страницу

**Примеры:**
```
GET /api/books/?category=test&year_min=2020&year_max=2023
GET /api/books/?author=1&binding_type=hard
GET /api/books/?search=война&ordering=title
GET /api/books/?libraries=1&libraries=2  # Книги из библиотек с ID 1 и 2
GET /api/books/?page=2  # Вторая страница (если книг > 30)
```

**Формат ответа с пагинацией:**

Если книг ≤ 30:
```json
{
  "count": 25,
  "results": [...],
  "paginated": false
}
```

Если книг > 30:
```json
{
  "count": 684,
  "next": "http://localhost:8000/api/books/?page=2",
  "previous": null,
  "results": [...],
  "paginated": true
}
```

### Создание книги
```
POST /api/books/
```
**Требуется авторизация:** Да (IsAuthenticated)

**Body (JSON):**
```json
{
  "title": "Название книги",
  "subtitle": "Второе название",
  "category": 1,
  "author_ids": [1, 2],
  "publication_place": "Москва",
  "publisher": 1,
  "year": 2023,
  "year_approx": "",
  "pages_info": "300 стр., ил.",
  "circulation": 5000,
  "language": 1,
  "language_name": "Русский",
  "binding_type": "hard",
  "binding_details": "Твердый переплет, красный",
  "format": "regular",
  "price_rub": "1500.00",
  "description": "Аннотация",
  "condition": "excellent",
  "condition_details": "",
  "seller_code": "A-1-23",
  "isbn": "978-5-123456-78-9",
  "library": 1,
  "status": "none",
  "normalized_image_urls": [
    "/media/temp/normalized/normalized_uuid1.jpg",
    "/media/temp/normalized/normalized_uuid2.jpg"
  ]
}
```

**Новые поля:**
- `circulation` (Integer, опционально) - Тираж книги в штуках
- `language` (Integer, опционально) - ID языка текста книги
- `language_name` (String, опционально) - Название языка (например, "Русский"). Если язык не найден, будет создан новый
- `library` (Integer, опционально) - ID библиотеки. Если не указан, автоматически назначается первая библиотека пользователя
- `status` (String, опционально) - Статус книги (none, reading, read, want_to_read, want_to_reread). По умолчанию: "none"
- `normalized_image_urls` (Array[String], опционально) - Список путей к нормализованным изображениям из временной директории. Файлы будут перемещены в постоянное хранилище и созданы BookPage записи

**Примечание:** 
- `author_ids` - список ID авторов (до 3-х)
- При создании книги нормализованные изображения автоматически перемещаются из `media/temp/normalized/` в `media/books/pages/processed/` и создаются BookPage записи
- Если `library` не указан, автоматически назначается первая библиотека пользователя

### Статистика фильтров
```
GET /api/books/stats/
```
**Query параметры:**
- `libraries` - список ID библиотек (можно указать несколько раз: `?libraries=1&libraries=2`)
- `category` - ID категории (опционально, для фильтрации по категории)
- `hashtag` - ID хэштега (опционально, для фильтрации по хэштегу)
- `search` - поисковый запрос (опционально, для фильтрации по поиску)

**Ответ:** `200 OK`
```json
{
  "status": {
    "none": 50,
    "reading": 28,
    "read": 66,
    "want_to_read": 49,
    "want_to_reread": 65
  },
  "with_reviews": 93,
  "with_electronic": 0,
  "recently_added": 258
}
```

**Примечание:** Endpoint возвращает статистику для всех книг в выбранных категориях и библиотеках (не только для текущей страницы). Используется для отображения счетчиков в фильтрах.

**Примеры:**
```
GET /api/books/stats/?libraries=1&libraries=2
GET /api/books/stats/?libraries=1&category=5
GET /api/books/stats/?libraries=1&hashtag=3&search=война
```

---

### Детали книги
```
GET /api/books/{id}/
```
**Ответ:** Полная информация включая изображения, электронные версии, страницы и даты прочтения

**Пример ответа:**
```json
{
  "id": 1,
  "title": "Название книги",
  "circulation": 5000,
  "language": 1,
  "language_name": "Русский",
  "reading_dates": [
    {
      "id": 1,
      "date": "2024-01-15",
      "notes": "Прочитал за один вечер"
    }
  ],
  ...
}
```

### Обновление книги
```
PUT /api/books/{id}/
PATCH /api/books/{id}/
```
**Требуется авторизация:** Да (IsOwnerOrReadOnly - только владелец книги)

**Body (JSON):**
```json
{
  "title": "Название книги",
  "subtitle": "Второе название",
  "category": 1,
  "author_ids": [1, 2],
  "publisher": 1,
  "publication_place": "Москва",
  "year": 2023,
  "year_approx": "",
  "pages_info": "300 стр., ил.",
  "circulation": 5000,
  "language_name": "Русский",
  "binding_type": "hard",
  "binding_details": "Твердый переплет, красный",
  "format": "regular",
  "condition": "excellent",
  "condition_details": "",
  "isbn": "978-5-123456-78-9",
  "description": "Описание книги",
  "status": "none",
  "library": 1,
  "cover_page": 123  // ID страницы (BookPage), которая будет использоваться как обложка
}
```

**Важные поля:**
- `cover_page` - ID страницы (BookPage), которая будет использоваться как обложка книги. Если не указано, обложка остается без изменений.
- `author_ids` - массив ID авторов (до 3-х)
- `language_name` - название языка (если язык не найден, будет создан новый)
- Все остальные поля опциональны (можно обновить только нужные)

**Ответ:** `200 OK`
Возвращается полная информация о книге в формате `BookDetailSerializer`, включая:
- Все поля книги
- `cover_page` - ID обложки (если назначена)
- `pages` - список всех страниц книги
- `electronic_versions` - список электронных версий
- `reviews` - список отзывов
- `reading_dates` - список дат прочтения

### Удаление книги
```
DELETE /api/books/{id}/
```

### Изображения книги
```
GET /api/books/{id}/images/           # Список изображений
POST /api/books/{id}/images/          # Загрузка изображения
```
**POST Body (multipart/form-data):**
- `image` - файл изображения
- `order` - порядок (1-20)

### Электронные версии книги
```
GET /api/books/{id}/electronic_versions/     # Список версий
POST /api/books/{id}/electronic_versions/    # Добавление версии
```
**POST Body (JSON или multipart):**
```json
{
  "format": "pdf",
  "url": "https://example.com/book.pdf"
}
```
или
```
format: pdf
file: <файл>
```

### Страницы книги
```
GET /api/books/{id}/pages/            # Список страниц
```
**Query параметры:**
- `status` - фильтр по статусу обработки (pending, processing, completed, failed)

### Загрузка страниц
```
POST /api/books/{id}/upload_pages/
```
**Body (multipart/form-data):**
- `pages` - массив файлов изображений

**Ответ:** `201 Created`
```json
{
  "message": "Загружено 3 страниц",
  "pages": [
    {
      "id": 1,
      "page_number": 1,
      "original_url": "/media/books/pages/original/page1.jpg",
      "processed_url": null,
      "processing_status": "pending"
    }
  ]
}
```

### Удаление страницы
```
DELETE /api/books/{id}/pages/{page_id}/
```
**Ответ:** `204 No Content`

### Обработка страниц
```
POST /api/books/{id}/process_pages/
```
**Body (JSON):**
```json
{
  "page_ids": [1, 2, 3]  // опционально, если не указано - обрабатываются все
}
```

### Нормализация страниц (для мастера создания книги)
```
POST /api/books/normalize-pages/
```
**Требуется авторизация:** Да (IsAuthenticated)

**Body (multipart/form-data):**
- `files` - массив файлов изображений (можно указать несколько раз)

**Ответ:** `200 OK`
```json
{
  "normalized_images": [
    {
      "id": "temp_uuid",
      "original_filename": "page1.jpg",
      "normalized_url": "/media/temp/normalized/normalized_uuid.jpg",
      "width": 1920,
      "height": 2560
    }
  ],
  "total": 3,
  "processed": 3,
  "errors": []
}
```

**Примечание:** Нормализованные изображения сохраняются во временную директорию `media/temp/normalized/` и должны быть перемещены в постоянное хранилище при создании книги.

### Автозаполнение данных книги через LLM
```
POST /api/books/auto-fill/
```
**Требуется авторизация:** Да (IsAuthenticated)

**Body (JSON):**
```json
{
  "normalized_image_urls": [
    "/media/temp/normalized/normalized_uuid1.jpg",
    "/media/temp/normalized/normalized_uuid2.jpg"
  ]
}
```

**Ответ:** `200 OK`
```json
{
  "success": true,
  "data": {
    "title": "Название книги",
    "subtitle": "Подзаголовок",
    "category_id": 160,
    "authors": ["Автор 1", "Автор 2"],
    "publisher_name": "Название издательства",
    "publication_place": "Москва",
    "year": 2023,
    "year_approx": null,
    "pages_info": "300 стр., ил.",
    "circulation": 5000,
    "language_name": "Русский",
    "binding_type": "hard",
    "binding_details": "Твердый переплет",
    "format": "regular",
    "condition": "excellent",
    "condition_details": null,
    "isbn": "978-5-123456-78-9",
    "description": "Полное описание книги..."
  },
  "confidence": 0.85,
  "error": null
}
```

**Ошибки:**
- `403 Forbidden` - OpenAI API недоступен в регионе (требуется VPN)
- `400 Bad Request` - неверный формат запроса
- `500 Internal Server Error` - ошибка обработки на сервере

**Примечание:** Использует OpenAI GPT-4o для анализа изображений страниц книги и извлечения структурированных данных.

---

## 5. Book Images (Изображения книг)

### Список изображений
```
GET /api/book-images/
```
**Query параметры:**
- `book` - фильтр по книге (ID)

### Создание изображения
```
POST /api/book-images/
```
**Body (multipart/form-data):**
- `book` - ID книги
- `image` - файл изображения
- `order` - порядок (1-20)

### Детали изображения
```
GET /api/book-images/{id}/
```

### Обновление изображения
```
PUT /api/book-images/{id}/
PATCH /api/book-images/{id}/
```

### Удаление изображения
```
DELETE /api/book-images/{id}/
```

### Удаление изображения через BookViewSet
```
DELETE /api/books/{id}/images/{image_id}/
```
**Ответ:** `204 No Content`

---

## 6. Book Electronic (Электронные версии)

### Список электронных версий
```
GET /api/book-electronic/
```
**Query параметры:**
- `book` - фильтр по книге (ID)
- `format` - фильтр по формату (pdf, epub, mobi, fb2, djvu, txt, rtf, doc, docx)

### Создание электронной версии
```
POST /api/book-electronic/
```
**Body (JSON или multipart):**
```json
{
  "book": 1,
  "format": "pdf",
  "url": "https://example.com/book.pdf",
  "file": null  // или загрузить файл
}
```

### Детали электронной версии
```
GET /api/book-electronic/{id}/
```

### Обновление электронной версии
```
PUT /api/book-electronic/{id}/
PATCH /api/book-electronic/{id}/
```

### Удаление электронной версии
```
DELETE /api/book-electronic/{id}/
```

### Удаление электронной версии через BookViewSet
```
DELETE /api/books/{id}/electronic_versions/{version_id}/
```
**Ответ:** `204 No Content`

---

## 7. Book Pages (Страницы книг)

### Список страниц
```
GET /api/book-pages/
```
**Query параметры:**
- `book` - фильтр по книге (ID)
- `status` - фильтр по статусу обработки

### Создание страницы
```
POST /api/book-pages/
```
**Body (multipart/form-data):**
- `book` - ID книги
- `page_number` - номер страницы
- `original_image` - файл изображения

### Детали страницы
```
GET /api/book-pages/{id}/
```

### Обновление страницы
```
PUT /api/book-pages/{id}/
PATCH /api/book-pages/{id}/
```

### Удаление страницы
```
DELETE /api/book-pages/{id}/
```

### Обработка страницы
```
POST /api/book-pages/{id}/process/
```
**Ответ:** Обработанная страница с processed_image

---

## Статусы ответов

- `200 OK` - Успешный запрос
- `201 Created` - Объект создан
- `400 Bad Request` - Ошибка валидации
- `404 Not Found` - Объект не найден
- `500 Internal Server Error` - Ошибка сервера

---

## Примеры использования

### Создание книги с авторами
```bash
curl -X POST http://localhost:8000/api/books/ \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Война и мир",
    "category": 1,
    "author_ids": [1, 2],
    "year": 1869,
    "publisher": 1
  }'
```

### Загрузка изображения
```bash
curl -X POST http://localhost:8000/api/books/1/images/ \
  -F "image=@cover.jpg" \
  -F "order=1"
```

### Поиск книг по диапазону дат
```bash
curl "http://localhost:8000/api/books/?year_min=2020&year_max=2023"
```

### Обработка страниц
```bash
curl -X POST http://localhost:8000/api/books/1/process_pages/ \
  -H "Content-Type: application/json" \
  -d '{"page_ids": [1, 2, 3]}'
```

---

---

## 10. Аутентификация (JWT)

### Получение токена
```
POST /api/token/
```
**Body:**
```json
{
  "username": "user",
  "password": "password"
}
```

**Ответ:** `200 OK`
```json
{
  "access": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9...",
  "refresh": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..."
}
```

### Обновление токена
```
POST /api/token/refresh/
```
**Body:**
```json
{
  "refresh": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..."
}
```

**Ответ:** `200 OK`
```json
{
  "access": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..."
}
```

**Использование:**
Добавьте заголовок в запросы:
```
Authorization: Bearer {access_token}
```

---

## 11. User Profiles (Профили пользователей)

### Свой профиль
```
GET /api/user-profiles/me/
PUT /api/user-profiles/me/
PATCH /api/user-profiles/me/
```
**Требует:** Аутентификация

**Ответ (GET):** `200 OK`
```json
{
  "id": 1,
  "user": {
    "id": 1,
    "username": "user",
    "email": "user@example.com",
    "first_name": "",
    "last_name": ""
  },
  "full_name": "Иван Иванов",
  "photo": null,
  "photo_url": null,
  "description": "Описание профиля",
  "libraries_count": 2,
  "books_count": 15,
  "created_at": "2025-11-03T12:00:00Z",
  "updated_at": "2025-11-03T12:00:00Z"
}
```

**Body (PUT/PATCH):**
```json
{
  "full_name": "Иван Иванов",
  "description": "Новое описание",
  "photo": "<file>"
}
```

### Список профилей
```
GET /api/user-profiles/
```
**Query параметры:**
- `user` - ID пользователя

### Детали профиля
```
GET /api/user-profiles/{id}/
PUT /api/user-profiles/{id}/
PATCH /api/user-profiles/{id}/
DELETE /api/user-profiles/{id}/
```

---

## 12. Libraries (Библиотеки)

### Список библиотек
```
GET /api/libraries/
```
**Query параметры:**
- `owner` - ID или username владельца

**Требует:** Аутентификация для создания/редактирования

**Ответ:** `200 OK`
```json
[
  {
    "id": 1,
    "owner": 1,
    "owner_username": "user",
    "name": "Библиотека в Москве",
    "address": "ул. Ленина, д. 1",
    "city": "Москва",
    "country": "Россия",
    "description": "Моя библиотека",
    "books_count": 10,
    "created_at": "2025-11-03T12:00:00Z",
    "updated_at": "2025-11-03T12:00:00Z"
  }
]
```

### Создание библиотеки
```
POST /api/libraries/
```
**Требует:** Аутентификация

**Body:**
```json
{
  "name": "Библиотека в Москве",
  "address": "ул. Ленина, д. 1",
  "city": "Москва",
  "country": "Россия",
  "description": "Моя библиотека"
}
```
**Примечание:** `owner` устанавливается автоматически (текущий пользователь)

### Свои библиотеки
```
GET /api/libraries/my-libraries/
```
**Требует:** Аутентификация

### Детали библиотеки
```
GET /api/libraries/{id}/
PUT /api/libraries/{id}/
PATCH /api/libraries/{id}/
DELETE /api/libraries/{id}/
```
**Требует:** Аутентификация, только владелец может редактировать/удалять

### Книги в библиотеке
```
GET /api/libraries/{id}/books/
```

---

## 13. Hashtags (Хэштеги)

### Список хэштегов
```
GET /api/hashtags/
```
**Query параметры:**
- `search` - поиск по названию
- `creator` - фильтр по создателю (ID или username)

**Ответ:** `200 OK`
```json
[
  {
    "id": 1,
    "name": "фантастика",
    "slug": "fantastika",
    "creator": 1,
    "creator_username": "admin",
    "books_count": 150,
    "created_at": "2025-11-03T10:00:00Z"
  }
]
```

### Хэштеги по категории (с частотой)
```
GET /api/hashtags/by_category/
```
**Query параметры:**
- `category_id` - ID категории (если не указан, возвращаются хэштеги всех книг)
- `libraries` - список ID библиотек для фильтрации (можно указать несколько раз: `?libraries=1&libraries=2`)
  - **ВАЖНО:** Если библиотеки не указаны, возвращается пустой список хэштегов
  - Если указаны библиотеки, возвращаются только хэштеги из книг этих библиотек
  - `count` - количество книг с этим хэштегом в выбранных библиотеках (и категории, если указана)

**Ответ:** `200 OK`
```json
{
  "hashtags": [
    {
      "id": 1,
      "name": "фантастика",
      "slug": "fantastika",
      "count": 25
    },
    {
      "id": 2,
      "name": "детектив",
      "slug": "detektiv",
      "count": 15
    }
  ],
  "max_count": 25,
  "min_count": 1
}
```
**Описание:**
- Возвращает только хэштеги, которые есть у книг в выбранных библиотеках (и категории, если указана)
- Если выбрана родительская категория, включает хэштеги из всех её подкатегорий
- `count` - количество книг с этим хэштегом в выбранных библиотеках
- Отсортированы по убыванию частоты (самые частые первыми)
- **Если библиотеки не указаны, возвращается пустой список** (не все хэштеги)

**Примеры:**
```
GET /api/hashtags/by_category/?libraries=1&libraries=2
GET /api/hashtags/by_category/?category_id=5&libraries=1
```

### Создание хэштега
```
POST /api/hashtags/
```
**Требует:** Аутентификация

**Body:**
```json
{
  "name": "фантастика"
}
```
**Примечание:** `creator` устанавливается автоматически, `slug` генерируется из `name`

### Детали хэштега
```
GET /api/hashtags/{slug}/
```

---

## 14. Book Reviews (Отзывы на книги)

### Список отзывов
```
GET /api/book-reviews/
```
**Query параметры:**
- `book` - ID или название книги
- `user` - ID или username пользователя

**Ответ:** `200 OK`
```json
[
  {
    "id": 1,
    "book": 1,
    "book_title": "Название книги",
    "user": 1,
    "user_username": "user",
    "rating": 5,
    "review_text": "Отличная книга!",
    "created_at": "2025-11-03T12:00:00Z",
    "updated_at": "2025-11-03T12:00:00Z"
  }
]
```

### Создание отзыва
```
POST /api/book-reviews/
```
**Требует:** Аутентификация

**Body:**
```json
{
  "book": 1,
  "rating": 5,
  "review_text": "Отличная книга!"
}
```
**Примечание:** 
- `user` устанавливается автоматически
- Если отзыв уже существует для этой книги и пользователя, он обновляется (upsert)

### Детали отзыва
```
GET /api/book-reviews/{id}/
PUT /api/book-reviews/{id}/
PATCH /api/book-reviews/{id}/
DELETE /api/book-reviews/{id}/
```
**Требует:** Аутентификация, только автор может редактировать/удалять

---

## 15. Books (Расширенные endpoints)

### Свои книги
```
GET /api/books/my-books/
```
**Требует:** Аутентификация

**Ответ:** Список всех книг текущего пользователя

### Прочитанные книги
```
GET /api/books/my-books/read/
```
**Требует:** Аутентификация

### Книги "Буду читать"
```
GET /api/books/my-books/want-to-read/
```
**Требует:** Аутентификация

### Передача книги
```
POST /api/books/{id}/transfer/
```
**Требует:** Аутентификация, только владелец

**Body (передача в библиотеку):**
```json
{
  "library": 1
}
```

**Body (передача другому пользователю):**
```json
{
  "user": 2
}
```

**Примечание:** При передаче в библиотеку `owner` остается прежним, меняется только `library`. При передаче пользователю меняется `owner`.

### Добавление хэштегов
```
POST /api/books/{id}/hashtags/
```
**Требует:** Аутентификация, только владелец

**Body:**
```json
{
  "hashtag_names": ["#фантастика", "#приключения"]
}
```

**Примечание:** 
- Хэштеги создаются автоматически, если их нет
- Максимум 20 хэштегов на книгу
- Хэштеги привязываются к текущему пользователю как создателю

### Отзывы по книге
```
GET /api/books/{id}/
```
В детальном ответе книги (`BookDetailSerializer`) включено поле `reviews` со всеми отзывами и `average_rating` (средний рейтинг).

**Примечание:** Для книг со статусом `read` и `want_to_reread` в ответе также включено поле `reading_dates` со всеми датами прочтения. В интерфейсе отображается дата первого прочтения (самая ранняя) рядом со статусом.

### Управление датами прочтения
```
GET /api/books/{id}/reading_dates/        # Список дат прочтения
POST /api/books/{id}/reading_dates/       # Добавление даты прочтения
DELETE /api/books/{id}/reading_dates/{date_id}/  # Удаление даты прочтения
```

**POST Body (JSON):**
```json
{
  "date": "2024-01-15",
  "notes": "Прочитал за один вечер"
}
```

**Ответ (POST):** `201 Created`
```json
{
  "id": 1,
  "book": 1,
  "date": "2024-01-15",
  "notes": "Прочитал за один вечер",
  "created_at": "2024-01-15T12:00:00Z",
  "updated_at": "2024-01-15T12:00:00Z"
}
```

**Ответ (DELETE):** `204 No Content`

**Примечание:** 
- Доступно только для книг со статусом `read` или `want_to_reread`
- Дата должна быть в формате `YYYY-MM-DD`
- Одна дата прочтения на книгу (unique constraint на `book` и `date`)

**Расширенные Query параметры для списка книг:**
- `owner` - ID или username владельца
- `library` - ID или название библиотеки (одна библиотека)
- `libraries` - список ID библиотек (несколько библиотек, например: `?libraries=1&libraries=2`)
- `status` - статус книги (`read`, `want_to_read`, `want_to_reread`, `none`)
- `hashtag` - поиск по хэштегу

**Примеры:**
```
GET /api/books/?owner=1
GET /api/books/?library=1
GET /api/books/?libraries=1&libraries=2  # Книги из нескольких библиотек
GET /api/books/?status=read
GET /api/books/?hashtag=фантастика
```

**Примечание:** Параметр `libraries` позволяет фильтровать книги по нескольким библиотекам одновременно. Если указано несколько библиотек, возвращаются книги из всех указанных библиотек.

### Нормализация страниц (для мастера создания книги)
```
POST /api/books/normalize-pages/
```
**Требует:** Аутентификация

**Content-Type:** `multipart/form-data`

**Body:**
- `files`: List[File] - загруженные изображения страниц

**Ответ:** `200 OK`
```json
{
  "normalized_images": [
    {
      "id": "uuid",
      "original_filename": "page1.jpg",
      "normalized_url": "/media/temp/normalized/normalized_uuid.jpg",
      "width": 1920,
      "height": 2560,
      "error": null
    },
    ...
  ],
  "total": 5,
  "processed": 5,
  "failed": 0,
  "errors": null
}
```

**Примечание:**
- Нормализация использует OpenCV для автоматического обнаружения границ документа
- Поддерживаются несколько методов обнаружения (Canny, Sobel, Laplacian, адаптивная бинаризация)
- Для белых документов на белом фоне используется fallback (границы изображения с отступом)
- Нормализованные изображения сохраняются во временную директорию `media/temp/normalized/`
- Если обработка файла не удалась, в поле `error` будет сообщение об ошибке
- Поля `width` и `height` содержат размеры нормализованного изображения

---

**Последнее обновление:** 2025-11-06
